...
public class SeleniumActionMethods {
	...
	/**
	 * 
	 * @param Key - Send this Key into the element found from the locator.
	 * @param locator
	 */
	public void type(final Keys key, final By locator) {
		WebElement element = finder.getElement(locator, 0);
		if(element!=null){
			highlightIfInDemoMode(element);
	...
	private void highlightIfInDemoMode(final WebElement element) {
		if(System.getProperty("SELENIUM_DEMO_MODE").toUpperCase().equals("TRUE")) {
			highlightElement(element, 0.5);
		}
	}
